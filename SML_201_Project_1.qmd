...
> We observed that the overall trend for Netflix subscribers over the years 2020 to 2024
...
dis_stock_with_market_cap <-  dis_stock |>
  mutate(market_cap = close * shrsout,
         month_movement = abs(market_cap - lag(market_cap))) 

dis_stock_with_market_cap <- dis_stock_with_market_cap |>
  mutate(month_movement = ifelse(is.na(month_movement), 0, month_movement))
...
greatest_monthly_movement <- dis_stock_with_market_cap |> 
  filter(month_movement == max(month_movement)) |>
  pull(date)

# the greatest movement of market cap was happened in Decemeber of 2020, specifically on 2020-12-10
paste("The greatest movement of market cap was happened in Decemeber of 2020, specifically on", greatest_monthly_movement)
